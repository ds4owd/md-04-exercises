...
```{r}
waste <- read_csv("data/waste-city-level-sml.csv") 
```
...
1.  Use the `count()` function to count the number of rows for each value of the `income_id` variable in `waste` object.
...
1.  Use the `c()` function to create a vector called `levels_income` with the following values: "HIC", "UMC", "LMC", "LIC".
